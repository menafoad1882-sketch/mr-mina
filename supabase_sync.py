...
def build_migration_sql():
    ...
    for table in TABLES:
        if table in _SPECIAL_SCHEMA:
            ...
            # id بتوليد تلقائي (identity): يسمح للجداول ذات المفتاح الطبيعي بأن تُرفع
            # بلا id فيولّده Supabase تلقائيًا (يمنع خطأ 23502 null id و23505 تكرار pkey).
            lines.append(f"create table if not exists {table} "
                         f"(id bigint generated by default as identity primary key);")
    lines.append("")
    lines.append("-- 1-ب) تحويل عمود id في الجداول القديمة إلى توليد تلقائي (identity)")
    lines.append("--     حتى يمكن رفع الصفوف بلا id فيولّده Supabase (يمنع null id / تكرار pkey).")
    lines.append("--     آمن و idempotent: يتخطّى لو كان identity بالفعل، ويضبط التسلسل على أكبر id.")
    for table in TABLES:
        if table in _SPECIAL_SCHEMA:
            continue
        lines.append(
...
_TABLE_BATCH = {
    ...
    "exams": 50,
    "wa_logs": 100,
    "settings": 5,         # قد تحوي صور شعارات/تخطيطات base64 كبيرة → دفعات صغيرة جدًا لتفادي 57014
    "results": 100,
    "exam_attempts": 100,
    "attendance": 200,
}
